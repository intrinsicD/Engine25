...
namespace Bcg::Graphics {
    ...
    struct DeviceFunctions {
        ...
        void (*wait_for_fences)(void *ctx, const std::vector<FenceHandle> &fences, bool wait_all, Uint64 timeout) = nullptr;
        void (*reset_fences)(void *ctx, const std::vector<FenceHandle> &fences) = nullptr;

        // Swapchain operations
        bool (*acquire_next_image)(void *ctx, SwapchainHandle, Uint64 timeout, SemaphoreHandle semaphore, FenceHandle fence,
        ...
        // ----------------------------------------------------------
        void (*multi_draw_indirect)(void *ctx, Uint32 mode, Uint32 type, const void *indirect, Uint32 draw_count, Uint32 stride) = nullptr;
        void (*multi_draw_arrays_indirect)(void *ctx, Uint32 mode, const void *indirect, Uint32 draw_count, Uint32 stride) = nullptr;
        // ----------------------------------------------------------
        // New: Buffer mapping functions.
        // ----------------------------------------------------------
    ...
    public:
        ...
        PipelineHandle create_pipeline(const PipelineDesc &desc) { return funcs_.create_pipeline(backend_context_, desc); }
        void destroy_pipeline(PipelineHandle handle) { funcs_.destroy_pipeline(backend_context_, handle); }
        RenderPassHandle create_render_pass(const RenderPassDesc &desc) { return funcs_.create_render_pass(backend_context_, desc); }
        void destroy_render_pass(RenderPassHandle handle) { funcs_.destroy_render_pass(backend_context_, handle); }
        FramebufferHandle create_framebuffer(const FramebufferDesc &desc) { return funcs_.create_framebuffer(backend_context_, desc); }
        void destroy_framebuffer(FramebufferHandle handle) { funcs_.destroy_framebuffer(backend_context_, handle); }
        DescriptorSetLayoutHandle create_descriptor_set_layout(const DescriptorSetLayoutDesc &desc) { return funcs_.create_descriptor_set_layout(backend_context_, desc); }
        void destroy_descriptor_set_layout(DescriptorSetLayoutHandle handle) { funcs_.destroy_descriptor_set_layout(backend_context_, handle); }
        DescriptorSetHandle create_descriptor_set(const DescriptorSetDesc &desc) { return funcs_.create_descriptor_set(backend_context_, desc); }
        void destroy_descriptor_set(DescriptorSetHandle handle) { funcs_.destroy_descriptor_set(backend_context_, handle); }
        CommandBufferHandle create_command_buffer(const CommandBufferDesc &desc) { return funcs_.create_command_buffer(backend_context_, desc); }
        ...
        SwapchainHandle create_swapchain(const SwapchainDesc &desc) { return funcs_.create_swapchain(backend_context_, desc); }
        void destroy_swapchain(SwapchainHandle handle) { funcs_.destroy_swapchain(backend_context_, handle); }

        void device_barrier(PipelineStage src_stage, PipelineStage dst_stage, AccessFlags src_access, AccessFlags dst_access) {
            funcs_.device_barrier(backend_context_, src_stage, dst_stage, src_access, dst_access);
        }
        bool is_feature_supported(DeviceFeature feature) const { return funcs_.is_feature_supported(backend_context_, feature); }
        ...
        // Swapchain operations
        bool acquire_next_image(SwapchainHandle swapchain, Uint64 timeout, SemaphoreHandle semaphore, FenceHandle fence, Uint32 *image_index) {
        ...
        // ----------------------------------------------------------
        // New: Multi-draw indirect draw call.
        // ----------------------------------------------------------
