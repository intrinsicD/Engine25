...
namespace Bcg::Graphics::Gui {
    void ShowGui(const char *label, const Vector<Real, 4> &v) {
        if(label){
            ImGui::Text("%s", fmt::format("{}: {} {} {} {}", label, v[0], v[1], v[2], v[3]).c_str());
        }else{
            ImGui::Text("%s", fmt::format("{} {} {} {}", v[0], v[1], v[2], v[3]).c_str());
        }
    }
    void ShowGui(const char *label, const Vector<Real, 3> &v) {
        if(label){
            ImGui::Text("%s", fmt::format("{}: {} {} {}", label, v[0], v[1], v[2]).c_str());
        }else{
            ImGui::Text("%s", fmt::format("{} {} {}", v[0], v[1], v[2]).c_str());
        }
    }
    void ShowGui(const char *label, const Vector<Real, 2> &v) {
        if(label){
            ImGui::Text("%s", fmt::format("{}: {} {}", label, v[0], v[1]).c_str());
        }else{
            ImGui::Text("%s", fmt::format("{} {}", v[0], v[1]).c_str());
        }
    }

    void EditGui(const char *label, Vector<Real, 4> &v){
    ...
    void EditGui(const char *label, Vector<Real, 3> &v){
        // Select the correct ImGuiDataType and format string:
        const ImGuiDataType data_type = std::is_same<Real, float>::value ? ImGuiDataType_Float : ImGuiDataType_Double;
        const char *step_format = std::is_same<Real, float>::value ? "%.3f" : "%.6lf";
        // Edit all N components at once:
    ...
    void EditGui(const char *label, Vector<Real, 2> &v){
        // Select the correct ImGuiDataType and format string:
        const ImGuiDataType data_type = std::is_same<Real, float>::value ? ImGuiDataType_Float : ImGuiDataType_Double;
        const char *step_format = std::is_same<Real, float>::value ? "%.3f" : "%.6lf";
        // Edit all N components at once:
